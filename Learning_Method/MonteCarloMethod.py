...
def getAction(env, q_table, observation, episode,enemies,type): # get action (array)  # フィールド外は再計算
    ...
    for i in range(2):
        if np.random.uniform(0, 1) > epsilon:  # e-greedy low is off
            ...
            c = 0
            while b!=True:
                b, d, ms, next_pos = env.judAc(enemies[i], x[c])
                if type == "nb":
                    lv = env.show()
                    try:
                        if lv[next_pos[0],next_pos[1]] == 6 or lv[next_pos[0],next_pos[1]] == friends[0] or lv[next_pos[0],next_pos[1]] == friends[1]:
                            c += 1
                            b = False
                        else:
                            c += 1
            ...
            while b!=True:
                ...
                b, d, ms, next_pos = env.judAc(enemies[i], pa)
                if type != "nb":
                    b = True
            a.append([d, ms, next_pos])
