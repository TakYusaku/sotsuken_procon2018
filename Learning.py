...
if __name__ == '__main__':
    ...
    """
        ...
        m = str(sys.exc_info())
        info_error = [Win1,Win2,float(Win1/num_episode),float(Win2/num_episode),float(sum(f_rr)/episode),float(sum(e_rr)/episode),fs,le_delta,m]
        ts.Log(fm,"error",info_error)
        print(m)
        ts.writeQtable(info[8], q_table,)
        ts.writeQtable(info[9], q_table_Enemy,)
        print("How many times did QL win, and What is WPCT of QL ?")
        w1 = str(Win1) + " , " + str(float(Win1/))
        print(w1)
        print("How many times did MCM win, and What is WPCT of MCM ?")
        w2 = str(Win2) + " , " + str(float(Win2/))
        print(w2)
        m = "finished time is " + str(now)
        print(m)
        ts.saveImage(fm,s,f_rr,e_rr,)
    """
